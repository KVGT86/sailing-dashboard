...
// --- 1. THE WIND ROUTE (UK Met Office 2km) ---
router.get(['/solent', '/weather'], async (req, res) => {
    console.log("--- FETCHING WIND (Forecast API) ---");
    try {
        // Correct Endpoint: v1/forecast (NOT v1/marine)
        const url = 'https://api.open-meteo.com/v1/forecast?latitude=50.80&longitude=-1.30&current=temperature_2m,relative_humidity_2m,wind_speed_10m,wind_direction_10m,wind_gusts_10m&models=ukmo_ukv&wind_speed_unit=kn&timezone=Europe/London';
        
        const response = await axios.get(url);
        
        // This MUST return a "current" object for your frontend to work
        res.json({
        ...
        res.json({ source: "Safe Mode", current: { wind_speed_10m: 12, wind_direction_10m: 210, temperature_2m: 15 } });
    }
});

// --- 2. THE TIDE ROUTE (Sea Level) ---
router.get(['/tides/solent', '/tides'], async (req, res) => {
    console.log("--- FETCHING TIDES (Marine API) ---");
    try {
        // Correct Endpoint: v1/marine
...
router.get(['/recommendation', '/team/recommendation'], async (req, res) => {
    try {
        ...
        res.json({ base: result.rows[0] || { jib_selection: 'J2+' }, recommended_sail: result.rows[0]?.jib_selection || 'J2+' });
