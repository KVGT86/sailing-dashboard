...
import SetupGuide from './components/SetupGuide'; // New Import
...
export default function App() {
  ...
  // --- ACTIONS (addSailor, deleteSailor, etc. stay same as your previous code) ---
  ...
  const toggleOnBoat = async (sailor) => {
    // Local toggle for weight calculation
    const updated = { ...sailor, on_boat: !sailor.on_boat };
    await axios.post(`${API_URL}/athletes`, updated);
    fetchData();
  };
    ...
    <div className="bg-slate-100 min-h-screen pb-10">
      <nav className="bg-[#1D1B44] text-white p-4 shadow-lg flex justify-between items-center border-b-4 border-[#ED1C24]">
        ...
        <div className="flex gap-4 text-[10px] font-bold uppercase">
          {['dashboard', 'guide', 'weather', 'fleet'].map(tab => (
                ...
                {roster.map(s => (
                  <div key={s.id} className="flex justify-between p-3 bg-slate-50 border rounded">
                    <div className="flex items-center gap-3">
                      <input type="checkbox" checked={s.on_boat} onChange={() => toggleOnBoat(s)} />
