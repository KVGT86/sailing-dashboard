...
const initDB = async () => {const express = require('express');
const cors = require('cors');
const { Pool } = require('pg');
const axios = require('axios');
const FitParser = require('fit-file-parser').default || require('fit-file-parser');
...
// Configuration
const UKHO_API_KEY = process.env.UKHO_API_KEY;
const SOUTHAMPTON_STATION_ID = '0065'; // Primary reference for Hamble/Solent
...
// --- HELPERS ---
const getWindMultiplier = async () => { 
  try { 
    const r = await pool.query("SELECT value FROM system_config WHERE key = 'wind_multiplier'"); 
    return parseFloat(r.rows[0]?.value || '1.0'); 
  } catch (e) { return 1.0; } 
};
...
const scrapeBuoy = async (stationUrl) => {
  try {
    const res = await axios.get(stationUrl, { timeout: 3000 });
    const windMatch = res.data.match(/Avg Wind.*?(\d+\.?\d*)\s*kn/i);
    const dirMatch = res.data.match(/Wind Dir.*?(\d+)/i);
    const gustMatch = res.data.match(/Gust.*?(\d+\.?\d*)\s*kn/i);
    
    if (windMatch && dirMatch) {
      return { 
        wind_speed: parseFloat(windMatch[1]), 
        wind_direction: parseInt(dirMatch[1]),
        gust: gustMatch ? parseFloat(gustMatch[1]) : parseFloat(windMatch[1]) * 1.2,
        is_live: true 
      };
    }
    return null;
...
// --- CORE TELEMETRY: UK MET OFFICE 2KM (UKV) ---
app.get(['/api/solent', '/api/weather/solent'], async (req, res) => {
  const mult = await getWindMultiplier();
  
  try {
    // 1. Priority: Live Scrape (Bramblemet)
    const bramble = await scrapeBuoy('https://www.bramblemet.co.uk');
    if (bramble) {
      return res.json({
        source: `Bramblemet (Live) [x${mult}]`,
        weather: { current: {
          wind_speed_10m: bramble.wind_speed * mult,
          wind_direction_10m: bramble.wind_direction,
          wind_gusts_10m: bramble.gust * mult,
          temperature_2m: 15
        }},
    ...
    // 2. Fallback: UK Met Office 2km Model (via Open-Meteo)
    const omRes = await axios.get('https://api.open-meteo.com/v1/forecast?latitude=50.80&longitude=-1.30&current=temperature_2m,relative_humidity_2m,wind_speed_10m,wind_direction_10m,wind_gusts_10m&models=ukmo_ukv&timezone=GMT');
    const om = omRes.data.current;
    
    res.json({
      source: `UK Met Office 2km (UKV) [x${mult}]`,
      weather: { current: om },
      timestamp: new Date().toISOString()
    });

  } catch (err) {
    res.json({ source: "Synthetic Fallback", weather: { current: { wind_speed_10m: 12 * mult, wind_direction_10m: 210, temperature_2m: 15 } } });
  }
});

// --- TIDAL DATA: UKHO ADMIRALTY ---
app.get(['/api/tides/solent', '/api/tides/portsmouth'], async (req, res) => {
  try {
    if (UKHO_API_KEY) {
      const tideRes = await axios.get(
        `https://admiraltyapi.azure-api.net/uktidalapi/api/V1/Stations/${SOUTHAMPTON_STATION_ID}/TidalPredictions`,
        { headers: { 'Ocp-Apim-Subscription-Key': UKHO_API_KEY } }
      );
      return res.json({ source: "UKHO Admiralty", data: tideRes.data });
    }
    
    // Fallback to Open-Meteo Marine
    const resTide = await axios.get('https://marine-api.open-meteo.com/v1/marine?latitude=50.79&longitude=-1.10&hourly=sea_level_height_msl&timezone=GMT');
    res.json({ source: "Open-Meteo Marine", data: resTide.data });
  } catch (err) { 
    res.json({ source: "Error", hourly: { time: [], sea_level_height_msl: [] } }); 
  }
});
